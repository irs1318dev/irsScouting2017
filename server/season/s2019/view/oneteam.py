...
def onet_g(team):
    ...
    hatches = data.query('task == ["getHatch", "csHatch", "rocketHatch1", "rocketHatch2", "rocketHatch3"]')
    hatches = hatches.drop(['attempts'], axis=1)
    pivot_hatch = hatches.pivot(index='match', columns='task', values='successes').fillna(0)
    bhtasks = ['csHatch', 'getHatch', 'rocketHatch1', 'rocketHatch2', 'rocketHatch3']
    ...
    cargo = data.query('task == ["getCargo", "csCargo", "rocketCargo1", "rocketCargo2", "rocketCargo3"]')
    cargo = cargo.drop(['attempts'], axis=1)
    pivot_cargo = cargo.pivot(index='match', columns='task', values='successes').fillna(0)
    bctasks = ['csCargo', 'getCargo', 'rocketCargo1', 'rocketCargo2', 'rocketCargo3']
    _addcol(pivot_hatch, bctasks)
    cds_cargo = bmodels.ColumnDataSource(pivot_cargo)
    ctasks = cds_cargo.column_names[1:]

    t1plot = plt.figure(x_range=matches, title="One Team Graph: " + team, plot_height=350, plot_width=700)

    t1plot.vbar_stack(htasks, x=btransform.dodge('match', -.17, range=t1plot.x_range), width=0.3, source=cds_hatches,
                      color=bpalettes.BuPu5, legend=[" " + x for x in htasks])
    t1plot.vbar_stack(ctasks, x=btransform.dodge('match', .17, range=t1plot.x_range), width=0.3, source=cds_cargo,
                      color=bpalettes.Oranges5, legend=[" " + x for x in ctasks])
    return t1plot

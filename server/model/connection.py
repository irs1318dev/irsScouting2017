...
both the sqlalchemy package and the pscyopg2 package. The and sqlalchemy
engine and pyscopg2 connection pool objects are created as soon as
connection.py is imported.

Module Attributes
-----------------

`server.model.connection.engine`: A sqlalchemy engine object that is
...
providing, re-using, and dispoing of database connections. In general,
use the server.model.connection.engine object for all scouting system
database connections. Sqlalchemy supports higher-level operations and
works with Pandas dataframes.

`server.model.connection.pool`: This module also facilitates database
operations using the psycopg2 Python package.
The `server.model.connection.pool` attribute is a pyscopg2 connection
pool object, which provides manages databsase connections similarly to
sqlalchemy engine objects. The psycopg package is used sparingly in the
scouting system, but there are some database operations that are
simpler to complete with psycopg2 than sqlalchemy. For example The
psycopg2 packaage is used to create a test database for unit testing.

`server.model.connection.db_params`: Production database connection
parameters are also stored in this module, in the db_params attribute.
Both the the sqlalchemy engine (`server.model.connection.engine`) and
psycopg2 connection pool (`server.model.connection.pool`) are
initialized with these parameters the first time this module is
implemented by the scouting system. There should not be any other
references to production database connection parameters anywhere else
in the scouting system -- this enables the IRS to change database
connection parameters by changing only this module. The keys for the
connections.db_params dictionary are:
...
--------
    ...
    # Useage of psycopg2 connection pool
    import Server.model.connection as smc

...
def test_upsert(tables_empty, testdb_conn):
    assert tables_empty
    ...
    count = testdb_conn.execute(sql_count).scalar()
    assert count == 2

    sql_sel = sqlalchemy.text("SELECT * FROM events;")
    teams = pandas.read_sql_query(sql_sel, testdb_conn)
    assert teams.shape == (2, 4)
    assert teams.name[0] == "wairs1"

    # Teardown
    delete_all_rows("events", testdb_conn)


def test_upsert_rows(tables_empty, testdb_conn):
    assert tables_empty

    smu.upsert_rows("events", "name", 25, "{0:0>3}-q")
    sql = "SELECT * FROM events;"
    matches = pandas.read_sql_query(sql, testdb_conn)
...
def test_upsert_cols(tables_empty, testdb_conn):
    assert tables_empty
    smu.upsert_cols("task_options", {"task_name": "na",
                                     "type": "capability",
                                     "option_name": "na"})
    sql = "SELECT * FROM task_options;"
    tasks = pandas. read_sql_query(sql, testdb_conn)
